...
class ImageToVideoModel:
    def __init__(self, model_path: str, device: str = "cuda"):
        ...
        self._num_frames = int(os.getenv("I2V_NUM_FRAMES", "25"))
        self._fps = int(os.getenv("I2V_FPS", "6"))
        self._steps = int(os.getenv("I2V_NUM_INFERENCE_STEPS", "25"))
        self._guidance = float(os.getenv("I2V_GUIDANCE_SCALE", "1.5"))
...
def normalize_frames_to_pil(frames: Any) -> list[Image.Image]:
    ...
    if isinstance(first, np.ndarray):
        ...
        for array in frames:
            ...
            out.append(Image.fromarray(array).convert("RGB"))
        return out

    if torch.is_tensor(first) or torch.is_tensor(frames):
        tensor = frames if torch.is_tensor(frames) else torch.stack(frames)
        tensor = tensor.detach().float().cpu()
        if tensor.ndim == 4 and tensor.shape[-1] in (1, 3, 4):  # T,H,W,C
            pass
        elif tensor.ndim == 4 and tensor.shape[1] in (1, 3, 4):  # T,C,H,W
            tensor = tensor.permute(0, 2, 3, 1)
        else:
            raise TypeError(f"Unexpected tensor shape: {tuple(tensor.shape)}")

        if float(tensor.max()) <= 1.0:
            tensor = tensor * 255.0
        tensor = tensor.clamp(0, 255).to(torch.uint8).numpy()
        return [Image.fromarray(tensor[i]).convert("RGB") for i in range(tensor.shape[0])]
