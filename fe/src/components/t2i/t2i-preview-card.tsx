...
export function T2IPreviewCard({ controller }: { controller: HomeController }) {
  const {
    busy,
    currentModelPath,
    currentModelLabel,
    ...
    <Card className="border-muted-foreground/10 shadow-sm">
      ...
      <CardContent className="space-y-4">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {currentLoras.length > 6 ? <Badge variant="outline">+{currentLoras.length - 6} more</Badge> : null}
        </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Button
                ...
                onClick={() => setPreviewState((prev) => ({ ...prev, index: Math.max(0, prev.index - 1) }))}
              >
                <ChevronLeft />
              </Button>
              <input
        ...
        {previewSrc.startsWith("blob:") ? (
          <Button asChild variant="secondary" className="w-full">
            <a href={previewSrc} download={previewDownloadFilename || "generated.png"}>
              Download
            </a>
          </Button>
        ) : null}
