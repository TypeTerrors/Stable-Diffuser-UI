...
func NewDownloaderService(hub *Hub, maxConcurrent, queueSize int, apikey, modelInfoUrl, downloadUrl, baseDir, token string, ctx context.Context) *DownloaderService {
	s := &DownloaderService{
		hub:     hub,
		baseDir: baseDir,
		token:   strings.TrimSpace(token),
		queue:   make(chan DownloadJob, queueSize),
		client:  huggingface.NewHfClient(ctx, downloadUrl, modelInfoUrl, apikey),
		ctx:     ctx,
	}
	s.group.SetLimit(maxConcurrent) // battery slots
	return s
}
...
func (d *DownloaderService) runJob(ctx context.Context, job DownloadJob) {
